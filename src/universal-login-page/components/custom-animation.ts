...
export const customAnimation = (ref: HTMLElement) => {
  autoAnimate(ref, (el: any, action: any, oldCoords: any, newCoords: any) => {
    let keyframes: any
    ...
    if (action === 'remove') {
      keyframes = [
        { transform: 'scale(1)', opacity: 1 },
        { transform: 'scale(0.9)', opacity: 1, offset: 0.33 },
        { transform: 'scale(0.9)', opacity: 0.1, offset: 0.5 },
        { transform: 'scale(0)', opacity: 0 },
      ]
    }
    if (action === 'remain') {
      ...
      const [widthFrom, widthTo, heightFrom, heightTo]: any =
